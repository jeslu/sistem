class ServiciosController < ApplicationController
  ...
  def index
    @servicios = Servicio.all.order(created_at: :desc).load_async
  ...
  def create
    ...
    if @servicio.save
        respond_to do |format|
          format.html { redirect_to servicio_path, notice: "Servicio fue creado exitosamente." }
          format.turbo_stream
        end
      else
  ...
  private
    ...
    def servicio_params
      params.require(:servicio).permit(:name_serv, :description, :costo_serv, :active)
    end
end
